...
class TestSpec(DBTestBase):
    ...
    def test_spec_links(self):
        ...
        '''
        links = self.test_app().get('/people?page[offset]=0&pj_include_count=1').json['links']
...
class TestPaging(DBTestBase):
    ...
    def test_no_start_single_sort_attrib(self):
        '''Should get page of people starting from beginning.'''
        data = self.test_app().get(
            '/people?sort=name&page[limit]=2'
        ).json['data']
    ...
    def test_after_double_sort_deeprel(self):
        '''Should get posts where blog.owner.name is after bob and the id is lower than 6 (after 6 reversed)'''
        js = self.test_app().get(
    ...
    def test_count_with_after(self):
        '''Use page[after] and get a count of available results.'''
        js = self.test_app().get(
            '/people?sort=name&page[after]=bob&pj_include_count=true'
        ).json
...
class TestBugs(DBTestBase):
    ...
    def test_19_last_negative_offset(self):
        ...
        last = self.test_app().get(
            '/posts?pj_include_count=1&filter[title:eq]=frog&page[offset]=0'
        ).json['links']['last']
        offset = int(
